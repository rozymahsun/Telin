 ...
 */
package sg.telin.controller.login;
...
public class ZkLoginDialogCtrl extends WindowBaseCtrl implements Serializable {
	...
	protected Textbox txtbox_OpCode; // autowired
	...
	public void onCreate$loginwin(Event event) throws Exception {
		...
		this.txtbox_Username.addEventListener("onOK", new LoginListener());
		this.txtbox_Password.addEventListener("onOK", new LoginListener());
	...
	private void doLogin(){
		int status=loginService.loginAccepted(txtbox_Username.getValue(), txtbox_Password.getValue(), "");
		if (status==0){
			Executions.sendRedirect("pages/index.zul");
			User userLogin = loginService.getUserlogin();
			Sessions.getCurrent().setAttribute(Constants.USER_SESSION,userLogin);

		}else
			Executions.sendRedirect("zkloginDialog.zul?login_error=1");
	}
