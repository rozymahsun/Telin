...
import sg.telin.common.menu.dropdown.ZkossDropDownMenuFactory;
import sg.telin.common.menu.tree.ZkossTreeMenuFactory;
import sg.telin.controller.IndexCtrl;
import sg.telin.controller.util.WindowBaseCtrl;
...
public class MainMenuCtrl extends WindowBaseCtrl implements Serializable {
	...
	public void onCreate$mainMenuWindow(Event event) throws Exception {
		 ...
		 */
		System.out.println("  onCreate$mainMenuWindow ");
	...
	private void createMenu() throws InterruptedException {
		...
		toolbarbutton.setImage("/images/icon-folder-open.png");
		...
		toolbarbutton.setImage("/images/icon-folder-close.png");
		...
		showPage("/WEB-INF/pages/userList.zul", "menu_Item_Home", Labels.getLabel("menu_Item_Home"));
	}
	 ...
	 * @throws InterruptedException
	 */
	public void showPage(String zulFilePathName, String tabID, String tabLabel) throws InterruptedException {
		...
		try {
			...
			if (workWithTabs == 1) {
				...
				if (checkTab == null) {
					 ...
					 */
					if(zulFilePathName.contains("?")){
						String newZul=zulFilePathName.replace("?", ",");
						HashMap<String, Object> map = new HashMap<String, Object>();
						map.put(newZul.split(",")[1].split("=")[0], newZul.split(",")[1].split("=")[1]);
						Executions.createComponents(newZul.split(",")[0], tabpanel, map);
					}else
	...
	public void onClick$btnMainMenuChange(Event event) throws Exception {
		...
		changeToTreeMenu.setImage("/images/refresh_blue.jpg");
